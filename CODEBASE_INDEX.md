...
| `7z` / `7-Zip` | External Binaries | System-installed or build-bundled 7-Zip engine used for automatic archive extraction on Windows. |
...
- **`FuckingFastExtractor`** (`core/extractors/fuckingfast.py`):
  - Cloudflare Turnstile session solver and direct link resolver using `seleniumbase` Undetected Driver (UC) in headless mode and in-browser HTMX fetch execution.
...
- **`~/.silverspoon.log`**:
  - Runtime exception and debug log file using `RotatingFileHandler` (5MB max size per log file with 3 historical backups).
